...
public class BuildVersionSanitizer : IPreprocessBuildWithReport
{
    ...
    private static string SanitizeShortVersion(string raw)
    {
        ...
        if (s.Length > AppleShortVerMaxLen) s = s.Substring(0, AppleShortVerMaxLen);
        if (s.EndsWith(".")) s = s.TrimEnd('.');
        if (string.IsNullOrEmpty(s)) s = "1.0.0";
        return s;
    }
    ...
    {
        if (code < 1) code = 1;
        if (code > AndroidVersionCodeMax) code = AndroidVersionCodeMax;
        return code;
    }
}
#endif